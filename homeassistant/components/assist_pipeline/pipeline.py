...
import time
from typing import Any, Literal, cast, Optional, Tuple
import wave
...
def _async_resolve_default_pipeline_settings(
    ...
    *,
    ...
    conversation_engine_id = get_conversation_engine_id(conversation_engine_id)

    # Find a matching language supported by the Home Assistant conversation agent
    pipeline_language, conversation_language = get_conversation_languages(hass, conversation_engine_id)

    stt_engine_id, stt_language = _get_stt_engine_and_language(hass, pipeline_language, stt_engine_id)

    tts_engine_id, tts_language, tts_voice = _get_tts_engine_language_and_voice(hass,tts_engine_id,pipeline_language)
...
#helper functions 
def get_conversation_engine_id(conversation_engine_id: Optional[str]) -> str:
    if conversation_engine_id is None:
        conversation_engine_id = conversation.HOME_ASSISTANT_AGENT
    return conversation_engine_id

def get_conversation_languages(hass:HomeAssistant, conversation_engine_id:str ) -> tuple[str,str]:
    ...
    if conversation_languages:
        return hass.config.language, conversation_languages[0]
    return "en", "en"

def _get_stt_engine_and_language(hass:HomeAssistant, pipeline_language:str, stt_engine_id:Optional[str]) -> Tuple[Optional[str], Optional[str]]:
    if stt_engine_id is None:
        stt_engine_id = stt.async_default_engine(hass)

    stt_engine = stt.async_get_speech_to_text_engine(hass, stt_engine_id) if stt_engine_id else None
...
def _get_tts_engine_language_and_voice(hass:HomeAssistant,tts_engine_id: Optional[str],pipeline_language:str) -> Tuple[Optional[str], Optional[str],Optional[str]]:
    ...
    if tts_engine:
        ...
        if tts_languages:
            ...
            return tts_engine_id, tts_languages[0], tts_voice
...
class PipelineRun:
    ...
    async def wake_word_detection(
        ...
        self._record_debug_queue()

        wake_word_vad = self._initialize_vad(wake_word_settings)
        ...
        return self._handle_result(result, audio_chunks_for_stt)
    ...
    def _record_debug_queue(self):
        """Record the debug queue for wake-word detection."""
        if self.debug_recording_queue is not None:
            self.debug_recording_queue.put_nowait(f"00_wake-{self.wake_word_entity_id}")

    def _initialize_vad(self, wake_word_settings:WakeWordSettings) -> VoiceActivityTimeout | None:
    ...
    def _initialize_audio_buffer(self, wake_word_settings:WakeWordSettings) -> deque[EnhancedAudioChunk] | None :
    ...
    def _check_for_duplicate_wake_word(self, result):
    ...
    def _record_wake_word_time(self, wake_word_phrase):
        """Record the wake word time to prevent duplicate detections."""
        self.hass.data[DATA_LAST_WAKE_UP][wake_word_phrase] = time.monotonic()

    def _process_queued_audio(self, result, audio_chunks_for_stt):
    ...
    def _handle_result(self, result, audio_chunks_for_stt):
        if result is None:
                wake_word_output: dict[str, Any] = {}
        else:
            self._check_for_duplicate_wake_word(result)
            self._record_wake_word_time(result.wake_word_phrase)
...
class PipelineInput:
    ...
    async def execute(self) -> None:
        ...
        stt_processed_stream = self._prepare_audio_stream()

        try:
            if current_stage == PipelineStage.WAKE_WORD:
                # wake-word-detection
                current_stage = await self._handle_wake_word_detection(stt_processed_stream, stt_audio_buffer)
            ...
            if self.run.end_stage != PipelineStage.STT:
                current_stage = await self._handle_intent_and_tts(current_stage, intent_input)
    ...
    ## helper functions
    def _prepare_audio_stream(self) -> AsyncIterable[EnhancedAudioChunk] | None:
    ...
    self, stt_processed_stream: AsyncIterable[EnhancedAudioChunk], stt_audio_buffer: list[EnhancedAudioChunk]
    ) -> PipelineStage | None:
        # wake-word-detection
    ...
    self, stt_processed_stream: AsyncIterable[EnhancedAudioChunk], stt_audio_buffer: list[EnhancedAudioChunk]
        ...
        if self.wake_word_phrase is not None:
            self._check_duplicate_wake_up()

        stt_input_stream = self._get_combined_audio_stream(stt_audio_buffer, stt_processed_stream)

        return await self.run.speech_to_text(self.stt_metadata,stt_input_stream)
    
    def _check_duplicate_wake_up(self) -> None:
    ...
    def _get_combined_audio_stream(
        self, stt_audio_buffer: list[EnhancedAudioChunk], stt_processed_stream: AsyncIterable[EnhancedAudioChunk]
    ) -> AsyncIterable[EnhancedAudioChunk]:
        """Combine buffered audio with live streamed audio."""
        if not stt_audio_buffer:
            return stt_processed_stream
        
        async def buffer_then_audio_stream() -> AsyncGenerator[EnhancedAudioChunk, None]:
            # Yield buffered audio first
            for chunk in stt_audio_buffer:
                yield chunk
            
            # Then stream the live audio
            async for chunk in stt_processed_stream:
                yield chunk
        
        return buffer_then_audio_stream()
    
    async def _handle_intent_and_tts(
    self, current_stage: PipelineStage, intent_input: str | None
    ) -> PipelineStage:
    ...
    def _handle_pipeline_error(self, err: PipelineError) -> None:
...
### helper function
    def _validate_input_requirements(self) -> None:
    ...
    def _collect_prepare_tasks(self, start_stage_index: int, end_stage_index: int) -> list:
        prepare_tasks = []

        if self._is_stage_in_range(PipelineStage.TTS, start_stage_index, end_stage_index):
            prepare_tasks.append(self.run.prepare_wake_word_detection())
    ...
    def _is_stage_in_range(self, stage: PipelineStage, start_index: int, end_index: int) -> bool:
        """Check if a pipeline stage is within the range of start and end stages."""
